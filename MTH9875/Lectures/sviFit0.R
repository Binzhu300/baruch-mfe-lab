...
obj <- function(sviparams){
    ...
    sviVar <- svi(tmp1,k);
    tmp <- sum((midVar-sviVar)^2,na.rm=T);
    return(tmp);
};

fit <- optim(sviGuess,obj,method="L-BFGS-B",lower=c(-1000,0,0.00000001,-.999,-10),upper=c(+1000,100,100,+.999,+10));
fit$par <- fit$par*c(t,t,1,1,1);
