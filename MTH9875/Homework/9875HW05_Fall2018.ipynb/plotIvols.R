...
plotIvols <- function(ivolData,sviMatrix=NULL,slices=NULL){
	...
	for (slice in slices){
			...
			plot(k,bidVol,col="red",pch=24,cex=.5, xlim=xrange,ylim=yrange,main=paste("T =",format(t,digits=2,nsmall=2)),xlab="Log-Strike",ylab="Implied Vol.");
par(new=T);
plot(k,askVol,col="blue",pch=25,cex=.5,xlim=xrange,ylim=yrange,main=NA,xlab=NA,ylab=NA);
			...
			atmCurv[slice] <- (volInterp(sig/10)+volInterp(-sig/10)-2*atmVol[slice])/(2*(sig/10)^2)
		}
	par(mfrow=c(1,1),mex=1)
	par(new=F)
	return(list(expiries=expDates,atmVol=atmVol,atmSkew=atmSkew,atmCurv=atmCurv))
}
